			...
			if(m_data[current+1] =='/'){//end tag
				...
				while (m_data[next] != '>'){
					...
					if (m_data[next] != '>') {//attribute
						...
						size_t startOfAttributeData = m_data.find('\"', endOfAttributeName + 1);
						size_t endOfAttributeData = m_data.find('\"', startOfAttributeData + 1);
						m_nodeQueue.back()->setAttribute(std::string(&m_data[next], endOfAttributeData - next), std::string(&m_data[startOfAttributeData + 1], endOfAttributeData - startOfAttributeData));
					}
				}current = next + 1;
			}
		}else {//text node
